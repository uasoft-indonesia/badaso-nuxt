...
export class Badaso extends Hookable {
  ...
  getStoragePath (path) {
    ...
    const awsUrl = this.options.awsUrl
    const backendUrl = this.options.endpoint

    if (!path || path === '' || path === null) { return }

    if (driver === 's3') {
      if (!awsUrl) {
        throw new Error('No AWS_URL env detected. Please provide one in nuxt.config.js.')
      }

      return joinURL(awsUrl, path)
    }

    if (driver === 'public') {
      if (!backendUrl) {
        throw new Error('No BACKEND_BASE_URL env detected. Please provide one in nuxt.config.js.')
      }

      return joinURL(backendUrl, 'storage', path)
    }
  }
}
